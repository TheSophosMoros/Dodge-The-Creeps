...
{
  initChildren( ANIMATED_SPRITE );
  const auto animatedTypesArray = child.animatedSprite2D
                                  ->get_sprite_frames()
                                  ->get_animation_names();

  dist.param( std::uniform_real_distribution<>::param_type( 0, animatedTypesArray.size()-1 ) ); // NOLINT(*-narrowing-conversions)

  child.animatedSprite2D->set_animation( animatedTypesArray[ dist( rng ) ] ); // NOLINT(*-narrowing-conversions)
...
void Mob::_bind_methods()
{
  ...
  ClassDB::bind_method( D_METHOD( "_on_VisibilityNotifier2D_screen_exited" ),
                        &Mob::onVisibilityNotifier2DScreenExited );
}
